...
class Frontier:
    ...
    def select_node(self):
        ...
        if algorithm == "DFS":
            ...
            # last thing on the list out first
        ...
        if algorithm == "HS":
            ...
            def get_heuristic(item):
                row_state, col_state = item.state
                heuristic = (abs(row_goal - row_state) +
                             abs(col_goal - col_state))
                print("HEURI", heuristic)
                return heuristic

            self.frontier = sorted(self.frontier, key=get_heuristic)
